...
server <- shinyServer(function(input, output) {
  ...
  output$rentplot <- renderPlot({
    ...
    rentset <- mutate(rentset,
      "2011" = round(sum(rentset[2:13]) / 12),
    ...
    return(plot(
      x = colnames(rentset), y = rentset, type = "b", col = "blue",
  ...
  output$salesplot <- renderPlot({
    ...
    salesset <- mutate(salesset,
      "2011" = round(sum(salesset[2:13]) / 12),
    ...
    return(plot(
      x = colnames(salesset), y = salesset, type = "b", col = "red",
  ...
  output$month <- renderText({
    ...
    rentset <- select(rentset, "RegionName", "2012", "2013", "2014", "2015", "2016", "2017", "2018")
    full_text <- paste0(
      "It would have taken approximately ", round(salesset[input$year] / rentset[input$year]),
      " months to buy a house with the amount of rent being paid."
    )
  })
  chosen_state_full <- reactive({
    validate(need(
      input$chosen_state_full %in% c(
